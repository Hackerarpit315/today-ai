...
def test_valid_approval():
    d=evaluate(req(action_type="send_email", target="alice@example.com", target_type="another_person", approval=approval())); assert d.permission_state is PermissionState.granted and d.approval_valid
...
def test_low_policy_can_allow():
    d=evaluate(req(policy={"allow_low_risk_without_approval":True})); assert not d.permission_required and d.permission_state is PermissionState.not_required
