...
def test_valid_request():
    response = post({"content": "Mujhe kal college jana hai.", "current_datetime": DT})
...
def test_response_structure():
    body = post({"content": "hello", "current_datetime": DT}).json()
    assert set(body) == {"request_id", "status", "current_stage", "pipeline", "errors"}
    assert set(body["pipeline"]) >= {
        "request_id", "status", "current_stage", "intent", "context",
        "research", "verification", "planning", "priority", "today",
        "errors", "stage_results",
    }
...
def test_empty_content_rejected():
    assert_safe_error(post({"content": "", "current_datetime": DT}))


def test_whitespace_content_rejected():
    assert_safe_error(post({"content": " \n\t ", "current_datetime": DT}))


def test_oversized_content_rejected():
    response = post({"content": "x" * 10001, "current_datetime": DT})
    assert_safe_error(response)


def test_missing_content_rejected():
    assert_safe_error(post({"current_datetime": DT}))


def test_extra_request_field_rejected():
    assert_safe_error(post({"content": "hello", "current_datetime": DT, "request_id": "x"}))


def test_missing_datetime_rejected():
    assert_safe_error(post({"content": "hello"}))


def test_timezone_aware_datetime_accepted():
    response = post({"content": "hello", "current_datetime": "2026-09-15T10:00:00Z"})
    assert response.status_code == 200


def test_naive_datetime_rejected():
    assert_safe_error(post({"content": "hello", "current_datetime": "2026-09-15T10:00:00"}))


def test_invalid_datetime_rejected():
    assert_safe_error(post({"content": "hello", "current_datetime": "not-a-date"}))
...
def test_orchestrator_success_is_returned(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", FakeOrchestrator)
    body = post({"content": "hello", "current_datetime": DT}).json()
...
def test_orchestrator_stage_failure_is_returned_safely(monkeypatch):
    ...
    result = PipelineResult(
        ...
        errors=[StageError(stage="verification", code="stage_error", message="Stage failed safely.")],
    )
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", FakeOrchestrator)
    response = post({"content": "hello", "current_datetime": DT})
    assert response.status_code == 200
    body = response.json()
...
def test_unexpected_internal_error_is_safe(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", BrokenOrchestrator)
    response = post({"content": "hello", "current_datetime": DT})
...
def test_no_stack_trace_on_orchestrator_error(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", BrokenOrchestrator)
    body = assert_safe_error(post({"content": "hello", "current_datetime": DT}), 500)
    assert "Traceback" not in str(body)
    assert 'File "' not in str(body)


def test_existing_module_1_api_still_works():
    response = client.post("/api/input", json={"input_type": "text", "content": "hello"})
    assert response.status_code == 200
    assert response.json()["status"] == "accepted"


def test_existing_module_behavior_unchanged_for_empty_input():
    response = client.post("/api/input", json={"input_type": "text", "content": ""})
    ...
    class FixedIdOrchestrator:
        def run(self, request):
            return PipelineResult(
                ...
                status="success",
                current_stage="today",
                stage_results={"input": {"content": request.text}, "today": {"current_datetime": request.current_datetime.isoformat()}},
            )

    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", FixedIdOrchestrator)
    payload = {"content": "same", "current_datetime": DT}
...
def test_explicit_datetime_reaches_orchestrator(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", InspectingOrchestrator)
    post({"content": "hello", "current_datetime": DT})
...
def test_client_cannot_control_request_id(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.assistant.Orchestrator", InspectingOrchestrator)
    response = client.post(ENDPOINT, json={"content": "hello", "current_datetime": DT, "request_id": "44444444-4444-4444-8444-444444444444"})
    assert response.status_code == 422
    assert "id" not in seen


def test_malformed_json_rejected():
    response = client.post(ENDPOINT, content=b"{bad", headers={"Content-Type": "application/json"})
    assert_safe_error(response)


def test_non_json_content_type_rejected():
    response = client.post(ENDPOINT, content=b"hello", headers={"Content-Type": "text/plain"})
...
def test_no_external_network_call_is_used():
    """
    The assistant endpoint must execute normally while the production
    application remains free of external network-client dependencies.
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if alias.name in forbidden_modules or root_name in {
                        "requests",
                        "httpx",
                        "aiohttp",
                        "urllib3",
                        ...
                        violations.append(
                            f"{source_file}: external network import '{alias.name}'"
                        )
                ...
                if module in forbidden_modules:
                    violations.append(
                        f"{source_file}: external network import '{module}'"
                    )
